...
export default function UserTopbar({ onMenuClick }: TopbarProps) {
  const [role, setRole] = useState<string | null>(null);
  const [portalUser, setPortalUser] = useState<{ name?: string; role?: string } | null>(null);
      ...
      <div className="flex h-[64px] w-full items-center gap-3 px-3 sm:px-4 lg:px-6 xl:px-8">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="hidden truncate text-xs text-slate-500 sm:block">
            Secure account dashboard, reports and payment access
          </p>
        </div>
