...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!filters.partyId) {
      const first = partyOptions[0].id;
      setFilters((prev) => ({ ...prev, partyId: first }));
      setDraftFilters((prev) => ({ ...prev, partyId: first }));
      return;
    }

    const exists = partyOptions.some((party) => party.id === filters.partyId);
    if (!exists) {
      const first = partyOptions[0].id;
      setFilters((prev) => ({ ...prev, partyId: first }));
      setDraftFilters((prev) => ({ ...prev, partyId: first }));
    }
  }, [partyOptions, filters.partyId]);
  ...
  linkedSettlements.forEach((against) => {
   ...
   const filteredRows = useMemo(() => {
    ...
    rows = rows.filter((row) => row.partyId === filters.partyId);
  ...
  const onAccountBaseRows = useMemo(() => {
    ...
    return allRows
      .filter(
        (row) =>
          row.partyId === filters.partyId &&
          row.allocationType === "ON_ACCOUNT",
      )
      .slice()
      .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
  }, [allRows, filters.partyId, type]);

  const onAccountOpeningNet = useMemo(() => {
    if (type !== "on-account") return 0;
    if (!filters.partyId || !filters.dateFrom) return 0;
...
function handleExportPdf() {
  ...
  const partyName =
    partyOptions.find((party) => party.id === filters.partyId)?.name || "party";

  openPdfActionModal({
    url: `${process.env.NEXT_PUBLIC_API_BASE_URL}${endpoint}?${params.toString()}`,
    title: `${getPageTitle(type)} PDF`,
    fileName: buildSafePdfFileName(
      `${partyName} ${getPageTitle(type)}${needsDate ? ` ${filters.dateFrom} to ${filters.dateTo}` : ""}.pdf`,
    ),
  });
}
        ...
        <div className="p-3 sm:p-4 lg:p-0">
          {loading ? (
            ...
            <EmptyState
              ...
              description="Please Select party to view bill rows."
            />
...
>
  ...
  Party, date aur baaki filters yahin se control honge.
</p>
                    </div>
                    ...
                    <div className="space-y-3.5">
                      <div>
                        ...
                        <select
                          ...
                          className="h-10.5 w-full rounded-[18px] border border-slate-200 bg-white px-4 text-sm text-slate-700 outline-none transition-all duration-200 focus:border-violet-400 focus:ring-4 focus:ring-violet-100"
                        >
                          {partyOptions.map((party) => (
                            <option key={party.id} value={party.id}>
                              {party.name}
...
<div className="shrink-0 border-t border-violet-100 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(248,245,255,0.96))] px-4 py-3.5 sm:px-5">
  ...
  className="flex w-full items-center justify-between rounded-[22px] border border-violet-100 bg-white px-4 py-4 text-left transition hover:border-violet-200 hover:bg-violet-50/60 disabled:opacity-60 disabled:cursor-not-allowed"
>
  <div>
    <p className="text-sm font-semibold text-slate-900">
  ...
  onClick={handlePdfDownload}
  disabled={pdfDownloading}
  className="flex w-full items-center justify-between rounded-[22px] border border-violet-100 bg-white px-4 py-4 text-left disabled:opacity-60"
>
  <div>
    <p className="text-sm font-semibold text-slate-900">
      {pdfDownloading ? "Downloading..." : "Download"}
...
function AdminMobileBillWiseRow({
  row,
  bordered,
  onOpenAttachment,
}: {
 ...
 onOpenAttachment: (
  ...
  return createPortal(
    <div className="fixed inset-0 z-[400] flex items-end justify-center bg-black/30 p-2 backdrop-blur-sm sm:items-center sm:p-4">
      <div className="flex w-full max-w-4xl flex-col overflow-hidden rounded-t-[26px] border border-slate-200 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.18)] sm:max-h-[88vh] sm:rounded-[24px]">
...
function RowActionMenu({
  ...
  function onOpenAttachment(arg0: TransactionAttachmentRecord | undefined, arg1: string): void {
    throw new Error("Function not implemented.");
  }

  function openAttachmentInNewTab(arg0: TransactionAttachmentRecord) {
    throw new Error("Function not implemented.");
  }
