...
export default function UserLedgerPageClient() {
...
const [loggedInUser, setLoggedInUser] = useState<ReturnType<typeof getLoggedInUser>>(null);
...
const resolvedPartyId = useMemo(() => {
  ...
  // 🔥 TRY BOTH (auto detection)
  return (
    loggedInUser.partyId ||
    loggedInUser.id ||
    ""
  );
}, [loggedInUser]);
...
const partyTransactions = useMemo(() => {
  ...
  return transactions
    .filter((txn) => {
      return String(txn.partyId) === String(resolvedPartyId);
    })
    .slice()
    .sort(
      (a, b) =>
        new Date(a.voucherDate).getTime() -
        new Date(b.voucherDate).getTime(),
    );
}, [resolvedPartyId, transactions]);
  ...
  isOpeningLike: isOpeningLikeTransaction(txn),
    ...
    return filtered;
}, [filters.search, visibleTransactions]);
...
function exportPdf() {
  ...
  url.searchParams.set("partyId", resolvedPartyId);
  ...
  const fileLabel = loggedInUser?.name?.trim() || "ledger";
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0 xl:max-w-[620px]">
          ...
          <p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">
            Your ledger report with compact layout, opening and closing balances, and attachment access.
          </p>
...
) : !resolvedPartyId || partyTransactions.length === 0 ? (
  <EmptyState
    title="Unable to load your ledger"
    description="Your account mapping could not be resolved for ledger access. Please log in again and try once more."
  />
) : (
        <>
          ...
          {ledgerRows.length === 0 ? (
            <div className="space-y-4">
              <EmptyState
                ...
                description="No Transactions found. Please try to change filter."
              />
...
onChange={(e) => {
  ...
  setFilters((prev) => ({
    ...
    search: value.trim(),
        ...
        placeholder="Search by date, particulars, voucher type, voucher no..."
        className="h-10.5 w-full rounded-[18px] border border-slate-200 bg-white pl-10 pr-3 text-sm text-slate-700 outline-none transition-all duration-200 placeholder:text-slate-400 focus:border-violet-400 focus:ring-4 focus:ring-violet-100"
      />
    </div>
  </div>
